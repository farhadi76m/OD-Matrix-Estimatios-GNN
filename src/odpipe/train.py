"""
train_eval.py — Step 3: train the marginal GNN and evaluate.

Predict per-zone production/attraction (standardised-MSE) from edge features +
turn-count edge weights, then rebuild the OD with a Furness gravity prior.
Reuses the shared model factory and marginal loss/eval helpers; zone count and
data come from the config. Trains on CPU by default (avoids the GPU watchdog) and
is resumable (--max-seconds). Eval reports marginal corr, reconstructed-OD cell
...
class Pack(torch.utils.data.Dataset):
...
def make_loaders(graph, norm, batch_size):
    ds = {s: Pack(s, graph, norm) for s in ("train", "val", "test")}
...
def train(cfg, graph, norm, device, max_seconds):
    tc = cfg["train"]; mcfg = load_yaml("model")
    ...
    print(f"[train] feature_dim={info['feature_dim']} n_zones={z} "
    ...
    if last_p.exists():
        ...
        print(f"[train] resumed -> epoch {start} (best_mae={best_mae:.4f})", flush=True)
    ...
    for ep in range(start, tc["epochs"]):
        ...
        if bad >= tc["patience"]:
            print(f"[train] early stop @ {ep}"); break
        if max_seconds and time.time() - t0 > max_seconds:
            print(f"[train] PAUSED after epoch {ep} (budget); re-run to continue"); return False
    print(f"[train] training complete; best val_mae={best_mae:.4f}")
    return True
...
def evaluate(cfg, graph, norm, device):
    ...
    rep = {"n_test": len(true_od), "n_zones": z, "gravity_beta": beta, "reconstruction_ceiling": ceil,
...
def _geh_resim(cfg, graph, test_ds, pred_od, n=25):
    ...
    root = resolve(cfg["paths"]["samples_dir"]); meso = cfg.get("mesoscopic", False)
    to_h = 3600.0 / (3600 - 300)
    mg, cg = [], []
    for k in idx:
        s = pickle.load(open(root / f"sample_{test_ds.meta[k]['idx']:06d}.pkl", "rb"))
        r = simulate_od(pred_od[k], graph["zone_ids"], graph["edge_ids"], net, taz,
                        "/tmp/odpipe_geh", seed=cfg["sim_seed"], meso=meso)
        if not r["ok"]:
            continue
        mask = s["x_dyn"][:, 0] > 1.0
        mg.append(r["flow"][mask] * to_h); cg.append(s["x_dyn"][:, 0][mask] * to_h)
    if not mg:
...
def _print_report(rep):
    m = rep["marginals"]
    print(f"\n=== evaluation ({rep['n_zones']} zones, test n={rep['n_test']}) ===")
    ...
    for name, mm in rep["per_regime"].items():
        print(f"    {name:10s} cell_corr={mm['cell_corr']:.3f} rmse={mm['rmse']:.2f} "
