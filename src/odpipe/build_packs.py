"""
build_packs.py — Step 2b: pack per-sample pkls into train/val/test tensors.

Node features x[N,9] = static[length,lanes,speed_limit] + [flow,speed,density,
traveltime] + node[out_turn,in_turn]; edge weight ew[L] = per-link turn count;
target y[Z,Z]. Splits from index.json (stratified by tier). log1p+standardise
...
def _features(s, static, src, dst, n):
    """Node features [N,9] and per-link turn counts [L] for one sample."""
    ...
    x = np.concatenate([static, x_dyn, np.stack([out_turn, in_turn], 1)], 1)
